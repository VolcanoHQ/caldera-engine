...
def test_narrator_voice_query_falls_back_to_book_bible_tone(tmp_path, monkeypatch):
    # Narrator never gets a character_profiles.json entry (it's not a cast
    # member), so Express Mode's marketplace suggestion needs a different
    # source of description -- the book bible's tone/genre/era.
    _, pipeline_dir = _fixture_pipeline(tmp_path)
    monkeypatch.chdir(tmp_path)
    tier3_dir = pipeline_dir.parent / "tier3"
    _write_json(str(tier3_dir / "book_bible.json"), {
        "tone": "gothic and melancholic",
        "genre": "horror",
        "era_setting": "Victorian England, early 1800s",
    })

    query = console_api._character_voice_query("frankenstein", "Narrator")

    assert "gothic and melancholic" in query
    assert "horror" in query
    assert "Victorian England, early 1800s" in query


def test_narrator_voice_query_generic_fallback_without_book_bible(tmp_path, monkeypatch):
    _fixture_pipeline(tmp_path)
    monkeypatch.chdir(tmp_path)

    query = console_api._character_voice_query("frankenstein", "Narrator")

    assert query == "Narrator audiobook character voice"
